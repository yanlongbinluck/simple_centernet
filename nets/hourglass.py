...
class convolution(nn.Module):
    def __init__(self, k, inp_dim, out_dim, stride=1, with_bn=True):
        ...
        pad = (k - 1) // 2
        self.conv = nn.Conv2d(
            inp_dim, out_dim, (k, k), padding=(
                pad, pad), stride=(
                stride, stride), bias=not with_bn)
        ...
        self.skip = nn.Sequential(
            nn.Conv2d(
                ...
                stride=(
                    ...
                    stride),
                bias=False),
            nn.BatchNorm2d(out_dim)) if stride != 1 or inp_dim != out_dim else nn.Sequential()
        self.relu = nn.ReLU(inplace=True)
...
def make_layer(kernel_size, inp_dim, out_dim, modules, layer, stride=1):
    ...
    layers += [layer(kernel_size, out_dim, out_dim)
    ...
    def __init__(self, n, dims, modules):
        ...
        if self.n > 1:
            ...
            self.low2 = make_layer(
                3, next_dim, next_dim, next_modules, layer=residual)
        # curr_mod x residual，next_dim -> next_dim -> ... -> next_dim ->
        # curr_dim
        self.low3 = make_layer_revr(
            3, next_dim, curr_dim, curr_modules, layer=residual)
...
class exkp(nn.Module):
    def __init__(self, n, nstack, dims, modules, cnv_dim=256, num_classes=80):
        ...
        self.kps = nn.ModuleList([kp_module(n, dims, modules)
                ...
                nn.Sequential(
                    nn.Conv2d(
                        ...
                        (1,
                         1),
                        bias=False),
                    nn.BatchNorm2d(curr_dim)) for _ in range(
                    nstack -
                    1)])
    ...
    def forward(self, image):
        ...
        for ind in range(self.nstack):
            ...
            if self.training or ind == self.nstack - 1:
                outs.append([self.hmap[ind](cnv), self.regs[ind]
                            (cnv), self.w_h_[ind](cnv)])
...
get_hourglass = {
    'large_hourglass': exkp(
        n=5, nstack=2, dims=[
            256, 256, 384, 384, 384, 512], modules=[
                2, 2, 2, 2, 2, 4]), 'small_hourglass': exkp(
                    n=5, nstack=1, dims=[
                        256, 256, 384, 384, 384, 512], modules=[
                            2, 2, 2, 2, 2, 4])}
