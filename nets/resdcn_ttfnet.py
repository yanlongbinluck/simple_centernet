...
class FPN(nn.Module):
    def __init__(self, c1,c2,c3,c4):
        ...
        self.skip_1 = nn.Sequential(
            ...
            nn.BatchNorm2d(c1, momentum=BN_MOMENTUM), # ttfnet中这里没有使用BN
            nn.ReLU(inplace=True),
            nn.Conv2d(c1, c1, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(c1, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv2d(c1, c1, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(c1, momentum=BN_MOMENTUM),
        )
        ...
        self.skip_3 = nn.Sequential(
            ...
            nn.BatchNorm2d(c3, momentum=BN_MOMENTUM),
        )

        self.up_2 = nn.Sequential(
            DCN(c2, c1,kernel_size=(3, 3), stride=1,padding=1, dilation=1, deform_groups=1),
            nn.BatchNorm2d(c1, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(c1,c1,kernel_size=4,stride=2,padding=1,bias=False),
            nn.BatchNorm2d(c1, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
        )
        self.up_3 = nn.Sequential(
            DCN(c3, c2,kernel_size=(3, 3), stride=1,padding=1, dilation=1, deform_groups=1),
            nn.BatchNorm2d(c2, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(c2,c2,kernel_size=4,stride=2,padding=1,bias=False),
            nn.BatchNorm2d(c2, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
        )
        self.up_4 = nn.Sequential(
            DCN(c4, c3,kernel_size=(3, 3), stride=1,padding=1, dilation=1, deform_groups=1),
            nn.BatchNorm2d(c3, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(c3,c3,kernel_size=4,stride=2,padding=1,bias=False),
            nn.BatchNorm2d(c3, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
        )
...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, head_conv, num_classes):
        ...
        self.fpn = FPN(c1 = 64,c2 = 128,c3 = 256,c4 = 512)
        if head_conv > 0:
            # heatmap layers
    ...
    def init_weights(self, num_layers):
        ...
        print('=> init fpn weights from normal distribution')
        for name, m in self.fpn.named_modules():
...
if __name__ == '__main__':
    ...
    net = get_pose_net(18, num_classes=80)

    input = torch.rand(1,3,512,512)
    out = net(input)

    print(out[0][0].size(),out[0][1].size(),out[0][2].size(),)
