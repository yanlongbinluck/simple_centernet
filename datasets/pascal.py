...
class PascalVOC_eval(PascalVOC):
    ...
    def convert_eval_format(self, all_bboxes):
        ...
        for i in range(self.num_samples):
            ...
            img_name = self.coco.loadImgs(
                ids=[img_id])[0]['file_name'].split('.')[0]
    ...
    def do_python_eval(self, detections, use_07=True):
        ...
        for i, cls in enumerate(VOC_NAMES[1:]):
            rec, prec, ap = self.voc_eval(
                detections[cls], self.annopath, self.imgsetpath %
                self.set_type, cls, cachedir, ovthresh=0.5, use_07_metric=use_07)
            aps += [ap]
