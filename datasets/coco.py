...
class COCO(data.Dataset):
    def __init__(self, data_dir, split, split_ratio=1.0, img_size=512):
        ...
        if 0 < split_ratio < 1:
            split_size = int(np.clip(split_ratio *
                                     len(self.images), 1, len(self.images)))
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(
            self.img_dir, self.coco.loadImgs(
                ids=[img_id])[0]['file_name'])
        ...
        #img = cv2.cvtColor(img,cv2.COLOR_BGR2RGB) # for imagenet backbone
        ...
        trans_fmap = get_affine_transform(
            center, scale, 0, [
                self.fmap_size['w'], self.fmap_size['h']])
...
class COCO_eval(COCO):
    ...
    def __getitem__(self, index):
        ...
        #image = cv2.cvtColor(image,cv2.COLOR_BGR2RGB) # for imagenet backbone
